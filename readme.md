...
[![Offline](https://img.shields.io/badge/Resilience-Offline--First_SQLite_WAL-orange.svg?logo=sqlite&logoColor=white)](#-offline-first-resilience--connectivity-blackout-mode)
...
<p align="center">
  <b>⚡ Sub-second multi-modal OCR • 🔬 5-Layer forensic tampering (ELA) • 👤 Hybrid AWS Rekognition & InsightFace ArcFace biometrics • 📶 Offline-first SQLite WAL buffer with auto-sync on reconnect • 🌐 Cross-checkpoint impossible-velocity face graphs • ⚖️ Automated frictionless clearance • 🔗 Immutable SHA-256 cryptographic audit ledger.</b>
</p>

---

[🌟 Executive Overview](#-executive-overview) • [✨ Key Capabilities](#-key-capabilities) • [📶 Offline-First Resilience](#-offline-first-resilience--connectivity-blackout-mode) • [👤 Hybrid Biometric Architecture](#-hybrid-biometric-architecture-aws--insightface) • [🖥️ Tactical Screening Console Flow](#️-tactical-screening-console-flow) • [📜 Multi-Document Forensic Engine](#-multi-document-forensic-engine) • [🏗️ System Architecture](#️-system-architecture) • [⚡ LangGraph Orchestration DAG](#-langgraph-orchestration-dag) • [📦 Unified Microservices](#-unified-microservices) • [🎯 Explainable Risk Engine](#-explainable-risk-engine-0100) • [🧪 Test Suite](#-test-suite--verification) • [🚀 Quick Start](#-quick-start)
...
### 📶 4. Offline-First Resilience (see full section below)
* 🗄️ **SQLite WAL Buffer:** All screening data is buffered locally in WAL-mode SQLite when Supabase is unreachable — zero 503 errors to the officer console.
* 🔄 **Auto-Sync on Reconnect:** Background loop flushes SQLite → Supabase every 30 seconds when connectivity is restored, including full image uploads.
* 🗑️ **Purge-After-Sync:** SQLite rows are **hard-deleted** after a confirmed Supabase write — no stale data accumulates locally.

### 🌐 5. Cross-Checkpoint Face Graph & Velocity Tracking
...
### 📜 6. Multi-Document & Regional Compatibility
...
## 📶 Offline-First Resilience & Connectivity Blackout Mode

TriPort is designed to operate at **remote land border crossings and air-gapped outposts** where cloud connectivity is intermittent. The system **never crashes or shows a 503** to the officer — it degrades gracefully and self-heals on reconnect.

### How it Works

```
╔══════════════════════════════════════════════════════════════════════╗
║              OFFLINE MODE (WiFi off / Supabase unreachable)          ║
╠══════════════════════════════════════════════════════════════════════╣
║  get_db_optional() detects failure → yields None (no crash)         ║
║                                                                      ║
║  Stage 1 (Document Scan)                                             ║
║    └─► Full pipeline snapshot + base64 image data URLs              ║
║        stored in SQLite: backend/data/orchestrator_offline.db        ║
║                                                                      ║
║  Stage 2 (Face Verification)                                         ║
║    └─► Reads scan from in-memory cache or SQLite                    ║
║        Runs local InsightFace ArcFace pipeline                       ║
║        Merges Stage 2 results into same SQLite row                  ║
║        Live capture stored as base64 data: URL (no upload needed)   ║
║                                                                      ║
║  Officer Decision (Approve / Reject)                                 ║
║    └─► Recorded in SQLite payload via record_offline_decision()     ║
║        In-memory cache updated for immediate UI feedback            ║
╠══════════════════════════════════════════════════════════════════════╣
║              RECONNECT (background sync every 30 seconds)            ║
╠══════════════════════════════════════════════════════════════════════╣
║  is_online() == True → sync_offline_screenings_to_postgres()        ║
║                                                                      ║
║  For each pending SQLite row:                                        ║
║    1. Upload base64 doc image   → Supabase Storage (document-images)║
║    2. Upload base64 face crop   → Supabase Storage (document-images)║
║    3. Upload base64 live selfie → Supabase Storage (live-captures)  ║
║    4. Upsert scan_events row    → Supabase PostgreSQL               ║
║    5. Populate child tables     → extracted_fields, tampering,      ║
║                                   face_verification_results,        ║
║                                   risk_results, officer_decisions    ║
║    6. DELETE SQLite row         → purge-after-sync (no stale data)  ║
╚══════════════════════════════════════════════════════════════════════╝
```

### What is Preserved Offline
| Data | Offline Storage | Synced to Supabase |
|:---|:---|:---|
| Document pipeline snapshot | SQLite `pending_screenings.payload` JSON | ✅ `scan_events` + all child tables |
| Document scan image | `data:image/jpeg;base64,...` in payload | ✅ Uploaded to `document-images` bucket |
| Extracted face crop | `data:image/jpeg;base64,...` in payload | ✅ Uploaded to `document-images` bucket |
| Live camera capture | `data:image/jpeg;base64,...` in payload | ✅ Uploaded to `live-captures` bucket |
| Officer decision | `payload.officer_decision` field | ✅ `officer_decisions` table |
| In-memory session cache | Python dict (process lifetime) | ✅ On sync |

### Test Data Reset Script
A utility script is provided to wipe all test data for clean re-testing:
```bash
cd backend
.venv/bin/python scripts/reset_test_data.py
```
This recursively deletes all objects from both Supabase Storage buckets, truncates all 10 screening PostgreSQL tables, and empties the local SQLite buffer — while preserving `users`, `checkpoints`, and `watchlist_entries`.
...
└────────────────────────────────────────────────────────────────────────────────────────┘
   ...
   Authorized Entry Badge + SHA-256 Ledger  Compact Dual-Source View (260px panels):
   Sequence Hash + Next Traveler Dispatch   Document Portrait vs Live Webcam Feed
                                            AWS Rekognition Match + AUTO-CLEAR
```
...
* **Compact Dual-Source View:** Source A (Document eMRTD Portrait) vs. Source B (Live Camera Stream) — fixed 260px panel height so metrics and decision buttons are always visible without scrolling.
* **AWS Rekognition Score Meter:** Dynamic comparison evaluated against the official $90.0\%$ threshold.
* **Retake Support:** Officers can retake the live photo at any time; the camera feed resumes instantly without page reload.
...
```
                                  ...
                                  │   (LangGraph DAG • Auth • get_db_optional Resilience)  │
                                  └─────┬──────────────┬──────────────┬──────────────┬─────┘
                                        │              │              │              │
              ┌─────────────────────────┼──────────────┼──────────────┼──────────────┴────────────────────────┐
              ▼                         ▼              ▼              ▼                                       ▼
  ...
  └───────────┬───────────┘ └──────────┬───────────┘ └──────────┬───────────┘ └───────────┬────────────┘ └───────────┬────────────┘
              ...
              └────────────────────────┴───────────────┬────────┴─────────────────────────┴──────────────────────────┘
                     ...
                     ▼                                  ▼                                     ▼
       ┌───────────────────────────┐    ┌───────────────────────────┐    ┌─────────────────────────────┐
       │   Supabase PostgreSQL     │    │     Supabase Storage      │    │  Local SQLite WAL Buffer    │
       │ (pgvector, Docs, Clusters)│    │(Signed URLs for JPG Scans)│    │ backend/data/*.db (offline) │
       └───────────────────────────┘    └───────────────────────────┘    └─────────────────────────────┘
                                                                                       │
                                                                          Auto-sync → purge on reconnect
```
...
graph TD
    ...
    FANOUT -->|Async Task| S3[Supabase Storage / Offline Base64]
    ...
    CLEAR --> AUDIT
    AUDIT --> COND_DB{Supabase Online?}
    COND_DB -->|Yes| DB_PERSIST[(💾 PostgreSQL State Commit)]
    COND_DB -->|No| SQLITE[(📦 SQLite Offline Buffer)]
    SQLITE -.->|On reconnect| DB_PERSIST
...
| :--- | :---: | :--- | :--- |
| **`orchestrator`** | `8000` | LangGraph DAG pipeline coordination, Supabase DB pooling, auth, offline resilience | `LangGraph`, `FastAPI`, `SQLAlchemy`, `psycopg3`, `sqlite3` |
...
SUPABASE_BUCKET_DOCUMENTS=document-images
SUPABASE_BUCKET_LIVE=live-captures
...
### 6. Reset Test Data (Development)
Wipe all screening data from Supabase and local SQLite for a clean test run:
```bash
cd backend
.venv/bin/python scripts/reset_test_data.py
```
> ⚠️ **Irreversible.** Preserves `users`, `checkpoints`, and `watchlist_entries`.

### 7. Verify Ledger Cryptographic Integrity
...
* 📶 **Offline Resilience:** No sensitive data is ever lost during connectivity blackouts — WAL-mode SQLite buffer ensures zero data loss with automatic Supabase sync on reconnect.
* 🗑️ **Local Data Hygiene:** `backend/data/` is excluded from version control (`.gitignore`) and purged automatically after successful cloud sync.
